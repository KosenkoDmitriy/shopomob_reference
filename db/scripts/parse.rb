...
if (!key.blank?)
  ...
  url = "http://maps.yandex.ru/?text=#{key_word_escaped}&sll=44.680171999999985,43.01867800002452&sspn=1.0848999023437322,0.16951489115989204&source=catalog&key=#{key}&output=json"
  uri = URI.parse(url)
  http = Net::HTTP.new(uri.host, uri.port)
  json_headers = {
  ...
  json_path = File.dirname(__FILE__)+"/db/scripts/json/#{key_word}.json"
